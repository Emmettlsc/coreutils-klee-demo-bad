...
/*   fputs (_("\ */
/* \n\ */
/* If -e is in effect, the following sequences are reco */gnized:\n\
\n\
"), stdout);
  fputs (_("\
  \\\\      backslash\n\
              ...
              if (c == '\\' && *s)
                {
                  switch (c = *s++)
                    ...
                    case 'x':
                      {
                        unsigned char ch = *s;
			// cockeel1: this is the error I introduced now
			// echo -e "\xG1" 
			// will not treat \xG1 as a literal string as it should
			// but instead misinterprets G as a valid hex digit leading to
			// unpredictible output
